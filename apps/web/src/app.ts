...
const widgetLibrary: WidgetInstance[] = [
...
function cloneWidget(key: string, suffix: string): WidgetInstance {
  const source = widgetLibrary.find((item) => item.key === key);
  if (!source) throw new Error(`Unknown widget ${key}`);
  return { ...source, id: `${source.id}-${suffix}` };
}

const templates: Record<string, WidgetInstance[]> = {
  command: [
...
const savedLocale = localStorage.getItem("nexus:locale:user-101") as Locale | null;
const savedTheme = localStorage.getItem("nexus:theme:user-101") as Theme | null;
...
function currentWorkspace(): UserWorkspace {
  const workspace = state.workspaces.find((item) => item.id === state.activeWorkspaceId);
  if (!workspace) throw new Error("Active workspace missing");
  return workspace;
}
...
function widgetMetric(key: string): string {
...
function widgetBody(widget: WidgetInstance): string {
...
function renderSafety(): string {
  if (state.safetySignals.length === 0) return "";
...
function renderWidget(widget: WidgetInstance): string {
...
function renderSectionSurface(section: string): string {
...
function renderMainContent(workspace: UserWorkspace): string {
...
function moveWidget(sourceId: string, targetId: string): void {
...
function bindEvents(): void {
  ...
  document.querySelector<HTMLSelectElement>("#locale-select")?.addEventListener("change", async (event) => {
    state.locale = (event.target as HTMLSelectElement).value as Locale;
  ...
  document.querySelector<HTMLSelectElement>("#workspace-select")?.addEventListener("change", (event) => {
    state.activeWorkspaceId = (event.target as HTMLSelectElement).value;
    render();
  });
  document.querySelector<HTMLSelectElement>("#instrument-select")?.addEventListener("change", (event) => {
    state.instrument = (event.target as HTMLSelectElement).value;
    render();
  });
  document.querySelectorAll<HTMLElement>("[data-action]").forEach((element) => {
    element.addEventListener("click", () => {
      ...
      if (action === "remove" && widgetId) currentWorkspace().widgets = widgets.filter((item) => item.id !== widgetId);
      if (action === "grow" && widget) widget.width = Math.min(24, widget.width + 2);
      if (action === "shrink" && widget) widget.width = Math.max(4, widget.width - 2);
      if (action === "add-widget") {
        const available = widgetLibrary.find((candidate) => !widgets.some((item) => item.key === candidate.key));
        if (available) widgets.push({ ...available, id: `${available.id}-${Date.now()}` });
      }
      ...
      if (action === "restore") currentWorkspace().widgets = templates.command?.map((item) => ({ ...item })) ?? [];
      render();
    });
  });
  document.querySelectorAll<HTMLElement>(".widget").forEach((element) => {
    ...
    element.addEventListener("drop", (event) => {
      ...
      if (draggedWidgetId && target) moveWidget(draggedWidgetId, target);
